...
async function handleSpotifyExport() {
    ...
    if (!currentPlaylistData || !currentPlaylistData.spotify_integration) {
        posthog.capture('spotify_export_error', {
            error_type: 'no_data',
            error_message: 'No playlist data available'
        });
    ...
    if (!playlistName) {
        posthog.capture('spotify_export_error', {
            error_type: 'validation',
            error_message: 'Empty playlist name'
        });
    ...
    if (trackIds.length === 0) {
        posthog.capture('spotify_export_error', {
            error_type: 'no_tracks',
            error_message: 'No Spotify tracks available'
        });
    ...
    posthog.capture('spotify_export_started', {
    ...
    currentExportBtn.textContent = '🔄 Creating Playlist...';
    ...
    try {
        ...
        if (data.success) {
            ...
            posthog.capture('spotify_export_success', {
                playlist_name: playlistName,
                track_count: trackIds.length,
            ...
            currentExportResult.innerHTML = `<strong>🎉 Success!</strong><br>${data.message}`;
        } else {
            ...
            currentExportResult.className = 'error';
            currentExportResult.innerHTML = `<strong>❌ Error:</strong><br>${data.error || 'Failed to create playlist'}`;
        }
        ...
        posthog.capture('spotify_export_error', {
            ...
            playlist_name: playlistName,
            track_count: trackIds.length,
            export_time_ms: Date.now() - exportStartTime
        });
        
        currentExportResult.className = 'error';
        currentExportResult.innerHTML = `<strong>❌ Error:</strong><br>Network error - please try again`;
    } finally {
